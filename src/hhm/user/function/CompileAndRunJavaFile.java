...
import hhm.user.pojo.CompileResult;
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			if (classStrArray.length != 3) {
				...
				String javaadd= req.getSession().getServletContext().getRealPath("");
                String pan = javaadd.substring(0, 1);
				System.out.print(javaadd);
				System.out.print(pan);
				File sourceFile = new File(javaadd + "\\java\\bin\\" + className + ".java");// 保存源代码
				...
				// 往控制台注入命令
				bw = new BufferedWriter(new OutputStreamWriter(process
						.getOutputStream()));
				bw.write(""+pan+":\n");
				bw.write("cd "+javaadd+" \\java\\bin \n");
				...
				compileResult.setUseMemory(useMemory);
				compileResult.setStartTime(startCurrentTime);
				compileResult.setEndTime(endCurrentTime);
				compileResult.setUseTime(useTime);

				req.setAttribute("result", compileResult);
				// req.setAttribute("msg", "编译成功,使用时间：" + useTime + "毫秒");//
				// 这个使用时间是大概的,不精确
			...
			req.setAttribute("msg",
					"格式不符合规范，请检查类名是否正确(如：public class YouClassName{}).错误信息:"
							+ e.getMessage());
		}

		req.setAttribute("result", compileResult);
		req.getRequestDispatcher("/xml/UserFunction/CompileResultXml.jsp")
				.forward(req, resp);
		...
		public void run() {
			...
			try {
				while ((content = br.readLine()) != null) {
					...
					if (!content.startsWith("E")&&!content.startsWith("D")&&!content.startsWith("M")&&!content.startsWith("(")&&!content.startsWith("版")) {
						content1 = content1+content;
						
					}

				}
				compileResult.setContent(content1);
			} catch (IOException e) {
				e.printStackTrace();
			}
